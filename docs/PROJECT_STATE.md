...
- El Dockerfile de `pos-api`/`pos-worker` instala con `--ignore-scripts` (`deploy/api/Dockerfile`), así que el `postinstall` de `@prisma/client` nunca corre: sin un `prisma generate` explícito antes de `tsc` sobre `packages/db`, el build de Docker falla con `TS2305: Module "@prisma/client" has no exported member 'PrismaClient'`. El Dockerfile ya lo corre explícito (`pnpm --filter @posbar/db run prisma:generate`) justo después de copiar `packages/db/prisma`; en local no se nota porque el cliente generado ya vive en el `node_modules` del desarrollador. También se agregó `apk add openssl` en build y runtime: el motor de Prisma en Alpine lo necesita en tiempo de ejecución.
